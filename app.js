...
function slideOut() {
    ...
    slideAnimation({
        ...
        props: {
            transition: "transform 0.4s cubic-bezier(0.74, -0.34, 1, 1.19),opacity 0.4s ease-out", transform: `translateX(${sliderData.direction < 0 ? "-100%" : "100%"})`, opacity: 0,
        }
    })
...
function slideIn(e){
    slideAnimation({
        el: slides[sliderData.slideInIndex],
        props: {
            transition: "transform 0.4s ease-out, opacity 0.8s ease-out",transform: "translateX(0%)",opacity: 1,
        }
    })
